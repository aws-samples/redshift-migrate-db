...
add_schemas()
{
	for schema_name in $(psql -h $SOURCE_PGHOST -p $SOURCE_PGPORT -d $SOURCE_PGDATABASE -U $SOURCE_PGUSER -t -A -c "SELECT schema_name FROM svv_all_schemas WHERE database_name = current_database() AND schema_type = 'local' AND schema_name NOT IN ${EXCLUDED_SCHEMAS}"); do
		...
		if [ "${count}" -eq "0" ]; then
			psql -h $SOURCE_PGHOST -p $SOURCE_PGPORT -d $SOURCE_PGDATABASE -U $SOURCE_PGUSER -t -A -c "ALTER DATASHARE ${SOURCE_SHARE_NAME} ADD SCHEMA \"${schema_name}\";" -e
		else
			echo "INFO: ${schema_name} already added to datashare ${SOURCE_SHARE_NAME}"
		fi
		psql -h $SOURCE_PGHOST -p $SOURCE_PGPORT -d $SOURCE_PGDATABASE -U $SOURCE_PGUSER -t -A -c "ALTER DATASHARE ${SOURCE_SHARE_NAME} ADD ALL TABLES IN SCHEMA \"${schema_name}\";" -e
	done
}
grant_datashare()
{
...
create_external_schemas()
{
	for schema_name in $(psql -h $SOURCE_PGHOST -p $SOURCE_PGPORT -d $SOURCE_PGDATABASE -U $SOURCE_PGUSER -t -A -c "SELECT schema_name FROM svv_all_schemas WHERE database_name = current_database() AND schema_type = 'local' AND schema_name NOT IN ${EXCLUDED_SCHEMAS}"); do
		...
		if [ "${count}" -eq "0" ]; then
			psql -h $TARGET_PGHOST -p $TARGET_PGPORT -d $TARGET_PGDATABASE -U $TARGET_PGUSER -t -A -c "CREATE EXTERNAL SCHEMA ext_${schema_name} FROM REDSHIFT DATABASE '${TARGET_DATASHARE_DATABASE}' SCHEMA '${schema_name}'" -e
		else
			echo "INFO: External schema ext_${schema_name} already exists."
		fi
	done
}
