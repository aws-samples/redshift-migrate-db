...
create_function()
{
	...
	for schema_name in $(psql -h $SOURCE_PGHOST -p $SOURCE_PGPORT -d $SOURCE_PGDATABASE -U $SOURCE_PGUSER -t -A -c "SELECT schema_name FROM svv_all_schemas WHERE database_name = current_database() AND schema_type = 'local' AND schema_name NOT IN ${EXCLUDED_SCHEMAS} ORDER BY schema_name"); do
		...
		for x in $(psql -h $SOURCE_PGHOST -p $SOURCE_PGPORT -d $SOURCE_PGDATABASE -U $SOURCE_PGUSER -t -A -c "SELECT p.prooid, p.proname FROM pg_proc_info p JOIN pg_namespace n ON p.pronamespace = n.oid JOIN pg_language l ON p.prolang = l.oid JOIN pg_user u ON p.proowner = u.usesysid WHERE n.nspname = '${schema_name}' AND l.lanname IN ('sql', 'plpythonu') AND u.usename <> 'rdsdb' ORDER BY p.proname"); do
			oid=$(echo ${x} | awk -F '|' '{print $1}')
			proname=$(echo ${x} | awk -F '|' '{print $2}')
			param_count=$(psql -h $SOURCE_PGHOST -p $SOURCE_PGPORT -d $SOURCE_PGDATABASE -U $SOURCE_PGUSER -t -A -c "SELECT CASE WHEN p.proargtypes = '' THEN 0 ELSE regexp_count(oidvectortypes(p.proargtypes), ',')+1 END FROM pg_proc_info p WHERE p.prooid = ${oid}")
			#get parameters
			if [ "${param_count}" -eq "0" ]; then
				...
				for y in $(seq 1 ${param_count}); do
					if [ "${y}" -eq "1" ]; then
						param=$(psql -h $SOURCE_PGHOST -p $SOURCE_PGPORT -d $SOURCE_PGDATABASE -U $SOURCE_PGUSER -t -A -c "SELECT COALESCE(p.proargnames[${y}], '') || CASE WHEN p.proargmodes IS NULL THEN ' IN ' WHEN (p.proargmodes[${y}]) = 'i' THEN ' IN ' WHEN (p.proargmodes[${y}]) = 'o' THEN ' OUT ' WHEN (p.proargmodes[${y}]) = 'b' THEN ' INOUT ' END || split_part(oidvectortypes(p.proargtypes), ',', ${y}) FROM pg_proc_info p WHERE p.prooid = ${oid}")
